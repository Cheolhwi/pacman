...
- **16.2 本地无法区分 A 与当前线、且 A 回归安全**：RANDOM23 49/49（+9.47 逐分历史一致）、defaultCapture 0-0 deadlock 等价、头对头 side-controlled 净值打平（A +0.135 vs 当前线 −0.135）。唯一偏向当前线的 frozenHS 近镜像（−1.6 vs −2.6）= 过敏尺子伪信号（Phase 15 已定性）。→ 原计划提交 A（**后被用户全量回滚取代，见下〈Phase 16 决策变更〉**）。

### Phase 16 决策变更：用户回滚至 May 11 纯 HS 基线（2026-06-13）

**决策**：用户直接把 `myTeam.py` 回滚到 **May 11 纯 HS 基线**（git `028dc94` == `65feab7`，1375 行），取代 16.1 的"在 14.1c 演化代码上翻 env flag"路径。比 GPT/16.1 设想的"变体 A"更彻底——不是关掉 QL/压迫的演化代码，而是回到 QL-guided A* 出现**之前**的整段历史。

**当前提交基线 ≠ 16.2 测的变体 A**（这点必须记清，否则误读 16.2 数字）：

- 变体 A = 演化代码（~2000 行）关 QL+压迫，**仍保留** Phase 4-13 的 HS 层老本（协作分工、防守记忆/失窃点、chokepoint 巡逻、bounded-A* 的 ghost step cost / 队友冲突 / 回家阈值调参等）。
- May 11 裸基线 = QL/Phase 全部出现前的版本，**只有**：PDDL 高层（attack/defence/patrol via `myTeam.pddl`）+ bounded-A* 低层 HS + `getCooperativeModeOverride` 协作模式分工 + `shouldReturnHome` + `lastEatenFood` 防守；`QLWeights` 结构在（QL 低层模式可选，默认走 HS）。
- **不含**：`qlGuidedAStar`、`pressureHomeEnabled`、Phase 13-15 全部防守机制/诊断、所有 env 开关（`QL_GUIDED`/`HS_PRESS_HOME`/`PACMAN_DEBUG`）、`dprint` 门控。10 个无条件 `print` 回归（per 16.0c 不致超时，仅卫生问题）。
- 故 **16.2 的数字刻画的是 A，不直接代表本基线**；本基线尚未被本地 A/B 过（裸基线缺 Phase 4-13 改进，对某些对手可能弱于 A，但这是用户的明确取舍——要一个干净的已知良好核心交服务器定标）。

**仍然有效的 Phase 16 结论**（与回滚无关）：16.0c（print 不致超时，打印机制相同）+ 方法论主线（服务器才是尺子、一次一变量、本地钝尺只排错/查回归）。**已失效**：16.0a 门控 + 16.1 env flag 的**代码**（被全量回滚取代，已不在 `myTeam.py`）。

**后续**：服务器先给 May 11 裸基线定标；若确认，16.3 的 B/D/E 应在**裸基线**之上一次一个加回（不是在 A 之上）。`frozenTeam`/`frozenHsTeam`/`frozenCurTeam` 均为演化代码副本，与提交基线不同源，仅作本地对手。
