...
日期：2026-06-13（Phase 16.0-16.1：回退纯 HS clean baseline，计划 + 代码审计；实验待跑）

目标：按 PLAN.md Phase 16，响应**服务器 PvP 池首次真实反馈**——当前提交版（14.1c 线，`qlGuidedAStar=True` + `pressureHomeEnabled=True`）胜率约 1/3、弱于纯 HS。先门控 print 排隐藏超时、把默认翻回纯 HS clean baseline，本地（对手 = frozenHsTeam + 当前线）只做超时/回归/归因，强弱定论交服务器。回退点 = HEAD（14.1c 终态）。

### 触发：真实尺子推翻本地结论

- Phase 15 收官写"14.1c 已是局部最优、提交配置不变"，但那是**本地钝尺**（staff/berkeley/bravo 被碾压、镜像过敏）下的结论。服务器池（强对手 + 黑盒泛化）说当前版弱于纯 HS——Phase 11-15 全程缺失的信号。
- 与 record 14 附加自洽：14.1b/14.1c 对 frozenHS 红方 = -2.20「不比 off 稳」。frozenHs 这把尺子其实早已与服务器同向，只是当时被"vs staff/bravo 全胜"盖过。

### 代码审计发现（本轮，未改行为）

- **每步 print 无条件写 stdout**：myTeam.py:370/371/377/391/411（plan 重算 2 + 每步 high/low action 3），非诊断门控。1s/步预算下 IO/超时为服务器低胜率头号嫌疑。
- **默认非纯 HS**：myTeam.py:156 `qlGuidedAStar=True` 硬编码无 env 开关；myTeam.py:213 `pressureHomeEnabled` 默认开。当前 = HS + QL 路径偏置 + 压迫式防守。
- **frozenHsTeam ≈ 变体 A 现成参照**：`qlGuidedAStar=False` 且无压迫逻辑，行为上即"纯 HS clean baseline"。

### 实现（16.0a + 16.1，已落地，flag-off 行为不变）

- **16.0a print 门控**：模块级 `DEBUG = bool(os.environ.get("PACMAN_DEBUG"))` + `dprint()`（走 stderr）。5 个每步 print 全改 dprint；其中两个大字符串 print（replan 的 OBJ/INIT/GOAL 与 PLAN）额外用 `if DEBUG:` 包住——`dprint("..."+str(objects))` 的参数在调用前就求值，光门控 print 仍会每次 replan 构造大字符串。提交不设 `PACMAN_DEBUG` → 零输出零字符串构造。
- **16.1 qlGuidedAStar env 门控**：`self.qlGuidedAStar = os.environ.get("QL_GUIDED","0") not in ("","0")`（默认 off）；`pressureHomeEnabled` 默认从 "1" 翻成 "0"。提交默认 = 纯 HS clean。变体经 env 解析验证：A(空)=off/off、A+QL、A+Press、当前线(QL_GUIDED=1 HS_PRESS_HOME=1)=on/on。
- 本地评估工具：新增 `frozenCurTeam.py`（env-immune 当前线，QL=True+pressure=True 硬编码）做头对头；`frozenHsTeam` 确认不读这两个 env → env 只调 myTeam。

### 16.0b 行为等价（门控前后逐位一致）

defaultCapture `-f` 同种子、当前线策略，PACMAN_DEBUG on vs off：stderr dprint 行数 **1498 vs 0**，结果均 **Tie game!**——门控对博弈零影响，确认 dprint 行为中性。

### 16.0c 超时实测（结论：print 不是服务器低胜率主因）

bloxCapture、`-c`（1s warn / 3s forfeit 强制）、策略恒为当前线以隔离 print 成本：

| 配置 | 10 局 wall（`-f` 同局隔离，两跑） | warnings / timeouts | 
| --- | --- | --- |
| prints-on（PACMAN_DEBUG=1） | 16.25 / 16.03 s | 0 / 0 |
| prints-off（提交态） | 15.65 / 15.67 s | 0 / 0 |

- print IO ≈ **0.5s / 10 局 ≈ 0.05s/局 ≈ ~0.1ms/步**（~1500 print/局 × ~32µs）。单步耗时本就 ~10-15ms，距 1s 警戒线 **3-4 个数量级**；全配置 **0 warning 0 timeout**。
- 不加 `-f` 时 prints-on(13.73s) 反比 prints-off(15.55s) 快——print 成本完全淹没在对局长度方差里。
- **结论**：GPT 的"print 隐藏超时"假设本地**不可复现**。门控仍保留（正确卫生、零代价、去掉 ~16k 行/10局 的 stderr 噪声），但服务器低胜率诊断**转向策略**（纯 HS vs 当前线，由 16.2 本地归因 + 16.3 服务器验证）。诚实保留：本地机快，单步 ~10-15ms 远低于 1s，即便服务器更慢 + stdout 走慢管道也难因 print 越线。

### 16.2 本地 A/B 归因（完成）

> myTeam 恒红方（固定侧）；frozenHsTeam/frozenCurTeam env-immune（env 只调 myTeam）。bloxCapture n=30 主尺，无 `-f`（吸引子采样，沿用 14 的 frozenHS 口径）。

**item1 — vs frozenHS 硬闯型（blox n=30，红方）：**

| 变体 | 红均分 | 红胜率 | min/max |
| --- | --- | --- | --- |
| A 纯HS clean | −2.6 | 0/30 | −4 / −1 |
| A+QL | −2.6 | 7/30 | −4 / +2 |
| A+Press | −2.3 | 0/30 | −4 / −1 |
| 当前线（QL+Press） | **−1.6** | 0/30 | −4 / 0 |

**item2-4 — 头对头 / 等价 / 回归：**

| 测试 | 结果 | 判定 |
| --- | --- | --- |
| A(红) vs 当前线(蓝) | −2.0（全 30 局恒 −2） | 蓝侧 +2 |
| 当前线(红) vs A(蓝) | −2.27（min −4 max 0） | 蓝侧 +2.27 |
| → A 侧均化净值 | **A +0.135 vs 当前线 −0.135（差 0.27）** | ✅ 头对头基本打平 |
| A vs frozenHS default（deadlock n=10） | 0.0（10/10 全平） | ✅ 纯 HS 镜像 0-0 签名，等价确认 |
| A RANDOM23 ×49 vs staff | **+9.47，49/49**（min 8 max 11） | ✅ 与历史逐分一致，基本盘无回归 |

### 16.2 结论：本地无法区分 A 与当前线，且 A 回归安全

- **回归安全**：A RANDOM23 49/49（+9.47，与 Phase 9 历史值逐分一致）、0 crash、0 timeout、defaultCapture 0-0 deadlock 等价签名全过。→ A 可安全提交。
- **头对头打平**：side-controlled 净值 A +0.135 vs 当前线 −0.135（差 0.27，噪声内）。blox 上胜负由**侧别**（蓝 +2~+2.27）而非配置决定 → A **不劣于**当前线。
- **frozenHS 近镜像边际偏向当前线（−1.6 vs A 的 −2.6）= 过敏尺子伪信号**：A-纯HS vs frozenHS-纯HS 是镜像局，−2.6 正压在纯 HS 镜像红方地板（−2.8，Phase 14 附加）；A+QL 的 7/30 上行是 record-14「从劣势侧偷分」的复现，Phase 15 已定性「只对镜像自己成立」。spread 仅 ~1 分 → **不构成保留 QL/压迫的理由**。
- **与服务器一致**：本地唯一「偏向当前线」的信号来自不可信镜像；可信尺子（RANDOM23、头对头）显示 A 不劣。服务器已裁定纯 HS > 当前线，本地无可信证据反驳 → **按计划提交 A，强弱定论交 16.3 服务器**。
- 副作用：`score` 已还原；新增 `frozenCurTeam.py`（本地评估工具，未提交）；日志 /tmp/p16_2/*.log。

**16.3 服务器提交阶梯**（真尺子，一次一变量）：

| 版本 | 池胜率 | vs 当前线 | 判定 |
| --- | --- | --- | --- |
| A（纯HS clean + prints gated） | 待填 | — | — |

### 决策变更（2026-06-13）：用户全量回滚至 May 11 纯 HS 基线

- 用户直接把 `myTeam.py` 回滚到 May 11 纯 HS 基线（git `028dc94` == `65feab7`，1375 行），**取代 16.0a 门控 + 16.1 env flag 路径**。提交基线 = 该裸版本（QL-guided A* 出现前的整段历史），**不是** 16.2 测的"变体 A"（A = 演化代码关 QL+压迫，仍保留 Phase 4-13 HS 改进）。
- 故本节 16.2 数字刻画的是 A，不直接代表当前提交基线；裸基线缺 Phase 4-13 改进，尚未本地 A/B 过。仍有效：16.0c（print 不致超时）+ 方法论主线。已失效：16.0a/16.1 的**代码**（已不在 myTeam.py）。
- 详见 PLAN.md〈Phase 16 决策变更〉。上方"16.3 提交 A"表作废——服务器先给 May 11 裸基线定标，B/D/E 之后在裸基线上加。
